<template>
  <div v-if="selected_content==contents[0]">
  ...
  <div v-if="selected_content==contents[3]">
    <div>Widget_name</div>
    <input type="text" v-model="new_widget_name"/>
    <div>
      List Media Source
    </div>
    <div v-for="source in proptemp.media_sources" style="display: flex;gap: 15px;margin: 40px">
      <input type="checkbox" v-model="source.select">
      <div>{{ source.id }}</div>
      <div>{{ source.name }}</div>
    </div>
    <button @click="create">Create Widget</button>
  </div>
  <div v-if="selected_content==contents[1]">
    <div id="container">
      <div id="content">
        <div id="content-left">
          <label>
          ...
          <div v-if="layout=='Slider-Squares'||layout=='Slider-Tiles'" class="line">
            <div>
              ...
              <input class="input_setting" v-model="per_slide" type="number" min="0">
            </div>
            <div>
        ...
        <div id="content-right">
          ...
          <div id="insta_feed">
            ...
            <div v-if="layout.includes('Grid')" id="images" :style="{
              ...
              <template v-for="(image,index) in allPosts">
                <div v-if="index<number_of_grid_posts"
                  ...
                  <img v-if="image.type=='IMAGE'" :style="{
                                    width: '100%',
                                    height: '100%'
                  ...
                  <div class="post_hover" v-if="image.hover&&image.type=='IMAGE'"
                       @click="show_post(image)">
                    <font-awesome-icon icon="fa-brands fa-instagram"
                                       style="color: white; height: 15%; width: 15%"/>
                  </div>
                  <div class="post_hover" v-if="image.hover&&image.type=='VIDEO'"
                       @click="show_post(image)">
        ...
        <div id="post_modal_container" style="display: flex">
          ...
          <div style="width: 100%; display: flex; flex-direction: column">
            <div
              ...
              <div
                  style="display: flex; justify-content: center; align-items: center;border: 1px solid #E2E2E2; border-radius: 50%; height: 40px; width: 40px">
                <font-awesome-icon icon="fa-brands fa-instagram"
      ...
      <Modal style="width: 50%; height: 100%;"
        ...
        <div style="height: 50%;overflow-y:auto;">
          <div class="product_row" style="display: flex; width: 100%;height: 100px ;
                              ...
                              align-items: center" v-for="product in filteredRowsRecommend"
               @click="product.tag=!product.tag">
...
export default defineComponent({
  ...
  data() {
    ...
    create() {
      this.$emit('createWidget', {
        list_media_sources_id: this.list_select_media_sources.map(e => e.id),
        name: this.new_widget_name
      })
    },
    edit(widget) {
    ...
    tagProduct(post) {
      this.tag_modal = true
      let current = this.allPosts.find(image => image == post)
    ...
    redirectToInstagramUser() {
      window.open('https://instagram.com/' + this.selected_post.instagram_business_account_username)
    },
    saveFeed() {
      this.selected_content = this.contents[0]
      let params = {
  ...
  computed: {
    list_select_media_sources() {
      return this.proptemp.media_sources.filter(e => e.select == true)
    },
    ...
    images() {
      return this.allPosts.filter((image, index) => index < this.number_of_posts)
    },
    ratio: function () {
      if (this.square) {
        return 1 / 1
    ...
    number_of_grid_posts() {
      if (this.layout == 'Grid-Tiles' || this.layout == 'Grid-Squares') {
        if (this.number_of_rows * this.number_of_columns > this.allPosts.length) {
...
<!--    <div id="navBar" style="height: 100px;">-->
<!--      <div-->
<!--          style="display: flex;gap:20px;position: absolute;right: 0;top:50%;height: 50%;color: #ffffff;transform: translate(0%, -50%);margin-right: 20px">-->
<!--        <div v-if="!show_chart" class="page_control" @click="showChart"-->
<!--             style="font-size: 22px; font-weight: 400;text-align: center;padding: 11px 11px;">Analytics-->
<!--        </div>-->
<!--        <div v-else class="page_control" @click="showChart"-->
<!--             style="font-size: 22px; font-weight: 400;text-align: center;padding: 11px 11px;">Settings-->
<!--        </div>-->
<!--        <div class="page_control"-->
<!--             style="font-size: 22px; font-weight: 400;text-align: center;padding: 11px 11px;"-->
<!--             @click="to_dashboard">Dashboard-->
<!--        </div>-->
<!--      </div>-->
<!--    </div>-->
<!--    <div v-if="show_chart" id="Chart">-->
...
<!--    create_feed() {-->
<!--      let self = this-->
<!--      axios.get('/instafeed/create/media_source').then((res) => {-->
...
<!--  mounted() {-->
<!--    let self = this-->
<!--    let allDays = new Array(5);-->
<!--    this.recent_days = Array.from({length: 5}, (_, i) => d.subtract(i, 'day').format(this.dateFormat));-->
...
<!--      }-->
<!--    })-->
<!--    axios.get('/instafeed/get').then((res) => {-->
...
<!--    //     self.per_slide = res.data.result.per_slide-->
...
<!--    //     // self.allPosts = res.data.media.data-->
<!--    //     self.feed_id = res.data.result.feed_id-->
<!--    //     self.analytics = res.data.result.analytic_days-->
<!--    //     self.allPosts.filter(image => image.hover = false)-->
<!--    //     axios.post('/instafeed/get/fb-data', {'feed_id': id}).then((res) => {-->
<!--    //       if (res.data.result.media) {-->
<!--    //         self.fb_username = res.data.result.fb_username-->
<!--    //         self.allPosts = res.data.result.media.data-->
...
<!--    //         const analyticsSet = new Set(self.analytics.map(e => e.day));-->
<!--    //-->
<!--    //         self.list_post_count = self.recent_days.map(day => {-->
<!--    //           const d = dayjs(day).format('YYYY-MM-DD');-->
<!--    //           return analyticsSet.has(d) ? self.analytics.find(e => e.day === d).analytics.posts : 0;-->
<!--    //         });-->
<!--    //         self.list_feed_count = self.recent_days.map(day => {-->
<!--    //           const d = dayjs(day).format('YYYY-MM-DD');-->
<!--    //           return analyticsSet.has(d) ? self.analytics.find(e => e.day === d).analytics.feeds : 0;-->
<!--    //         });-->
<!--    //         self.list_product_count = self.recent_days.map(day => {-->
<!--    //           const d = dayjs(day).format('YYYY-MM-DD');-->
<!--    //           return analyticsSet.has(d) ? self.analytics.find(e => e.day === d).analytics.products : 0;-->
<!--    //         });-->
<!--    //       }-->
<!--    //     })-->
<!--    //   })-->
