...
class SShopify(http.Controller):
    ...
    #         store_info = request.env['s.store.info'].sudo().search([('shop_url', '=', shop_url)], limit=1)
    #         list_price_discount = []
    #         for cart in store_info.combo_ids:
    #             original_total_price = data['original_total_price'] / 100
    #             for item in data['items']:
    #                 price_discount = 0
    #                 if str(item['product_id']) == cart.product_condition.product_id:
    #                     if item['quantity'] >= cart.quantity_condition:
    #                         if cart.is_percent:
    #                             item_price = (item['price'] * item['quantity']) / 100
    #                             price_discount = cart.value * item_price / 100
    #
    #                         else:
    #                             if cart.type_apply:
    #                                 item_price = (item['price'] * item['quantity']) / 100
    #                                 price_discount = item_price - cart.value
    #                                 if price_discount <= 0:
    #                                     price_discount = item_price
    #                             else:
    #                                 price_discount = cart.value
    ...
    # @http.route('/shopify/cart/update_line', type="json", auth="public", website=True, method=['POST'], csrf=False)
    # def post_line(self, **kw):
    #     if request.httprequest.data:
    #         res = json.loads(request.httprequest.data)
    #         shop_url = request.session['shop_url']
    #         line_exist = request.env['s.cart.products'].sudo().search([('product_id', '=', data['id'])], limit=1)
    #         store_info = request.env['s.store.info'].sudo().search([('shop_url', '=', shop_url)], limit=1)
    #         if combo_exist:
    #             combo_exist.write(vals)
    #         else:
    #             vals['store_id'] = store_info.id
    #             cart = combo_exist.create(vals)
    #         return {
    #             "status": True
    #         }
